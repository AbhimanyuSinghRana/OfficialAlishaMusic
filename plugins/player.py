...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text(
        (190, 670),
        f"Powered By: Abhimanyu X Khushbu (@Itz_VeNom_xD @candy_626)",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "** 😎 Qʋɘɘŋ ✌️ ʌɭɩsʜʌ 🥀 Ʀɘɑɗy ♥️ Ƭø ⭐ Ƥɭɑy 😎 ...**")

                except UserAlreadyParticipant:
                    pass
                except Exception:
                    await lel.edit(
                        f"**🎸 Ƥɭɘɑsɘ ❤️ Ɱɑŋʋɑɭɭy 🥀 Ʌɗɗ 💫 Ʌssɩsʈɑŋʈ 😔 Øɤ 🎸 Ƈøŋʈɑƈʈ ❤️ ʈø : @Itz_VeNom_xD 🥀** ")
    try:
...
async def pause(_, message: Message):
    ...
    await message.reply_photo(
                             photo="https://telegra.ph/file/763dedb3000b7f03168ea.jpg", 
                             caption="**💥 Aɭɩsʜʌ 🔈 Mʋsɩƈ🤞Nøω 🥀\n▶️ Ƥɑʋsɘɗ 🌷 ...**"
    )
...
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("**💥 Aɭɩsʜʌ 💞 Ɲøʈɦɩɳʛ 🔇\n🚫 Ƥɭɑyɩɳʛ 🌷 ...**")
    else:
